...
Scans `eval/runs/<model>/<seed_id>/slides/` for rendered decks, picks a
...
TEST_JSONL = REPO_ROOT / "data" / "hf" / "slides-sft-v0.test.jsonl"
QUEUE_PATH = Path(__file__).parent / "pair_queue.json"
...
def main() -> None:
    ...
    if not common_seeds:
        raise SystemExit("No seeds are rendered across ALL models — check eval/runs/")
    ...
    print(f"sampled seeds ({n}): {sampled}")
