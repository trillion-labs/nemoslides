...
Usage:
    ...
    uv run python -m nemoslides.eval.run --model nemotron-nano --limit 3
    uv run python -m nemoslides.eval.run --model gpt-5.4 --force
"""
...
from nemoslides._paths import DATA, REPO_ROOT, RENDERER, RESULTS
from nemoslides.eval.features import scan as scan_features
from nemoslides.eval.generate import MODELS, ModelSpec, generate
from nemoslides.eval.judge import judge_deck
from nemoslides.eval.rubric import DIMENSIONS, JUDGE_DIMENSIONS

RENDER_SCRIPT = RENDERER / "render.sh"
DEFAULT_TEST_JSONL = DATA / "hf" / "slides-sft-v0.test.jsonl"
...
def main() -> None:
    ...
    out_path = RESULTS / "eval" / f"{spec.name}_results.json"
