...
Run:
    uv run uvicorn nemoslides.blindtest.app:app --host 127.0.0.1 --port 8001
...
BT_DATA = RESULTS / "blindtest"
BT_DATA.mkdir(parents=True, exist_ok=True)
QUEUE_PATH = BT_DATA / "pair_queue.json"
DB_PATH = BT_DATA / "votes.db"
...
app = FastAPI(title="NemoSlides blindtest")
...
def load_queue() -> list[dict]:
    if not QUEUE_PATH.exists():
        raise HTTPException(500, "pair_queue.json missing — run `uv run python -m nemoslides.blindtest.build_pairs` first")
